...
# located
my $root_dir    = '/Users/cmadjar/Documents/McGill/PreventAD';
my $scripts_dir = $root_dir . '/Scripts/ASLquantification';
my $data_dir  	= $root_dir . '/Transfers/ASL/From_dcm_files/DATA_RELEASE_1.0_candidates';
# defaults
my $log_dir   = $scripts_dir . '/logs';
my $cbf_dir   = $data_dir    . '/ANALYSES/ASLquant_v2-054_Maverick_noFieldmapCorrection';
...
my $xml_file  = $scripts_dir . '/ROIaveragingParameters_v2.0_2013-10-28.xml';
my ($list, @args);
...
foreach my $row (@rows) {
    ...
    print "Cand mask directory: $cand_masks_dir\n";
    ...
    # read the XML file with Neurolens' options and get the list of pluggins to run 
	...
	if ($count < 1) {
		...
		my ($title_row) = &ASL::createTitleRow($roi_list);
	...
	foreach my $cbf (@$cbf_maps) {
		...
		my ($row) = &ASL::createSpreadsheetRow($roi_list, $candID,    $visit, 
											   $cbf,      $masks_dir, $plugin,
											   $options
											  );
